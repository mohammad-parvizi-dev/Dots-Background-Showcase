...
interface DotsBgGeneratorProps {
  onBack: () => void;
}

export default function DotsBgGenerator({ onBack }: DotsBgGeneratorProps) {
  ...
  const handleExportSettings = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `dot-background-settings.json`;
      link.click();
  ...
  const handleImportSettings = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (data.size !== undefined) setSize(Number(data.size));
        if (data.dotSize !== undefined) setDotSize(Number(data.dotSize));
        if (data.fill !== undefined) setFill(String(data.fill));
        if (data.maskColor !== undefined) setMaskColor(String(data.maskColor));
        if (data.showGlow !== undefined) setShowGlow(Boolean(data.showGlow));
        if (data.glowColor !== undefined) setGlowColor(String(data.glowColor));
        if (data.maskPosition !== undefined) setMaskPosition(data.maskPosition);
        if (data.maskStart !== undefined) setMaskStart(Number(data.maskStart));
        if (data.maskEnd !== undefined) setMaskEnd(Number(data.maskEnd));
        if (data.glowSize !== undefined) setGlowSize(Number(data.glowSize));
        if (data.glowSpread !== undefined) setGlowSpread(Number(data.glowSpread));
        if (data.interactive !== undefined) setInteractive(Boolean(data.interactive));
        if (data.mouseRadius !== undefined) setMouseRadius(Number(data.mouseRadius));
        if (data.mouseStrength !== undefined) setMouseStrength(Number(data.mouseStrength));
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-sm font-bold tracking-tight uppercase text-white">Dots Background</h1>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-[10px] font-mono tracking-wider text-indigo-400 hover:text-white flex items-center gap-1 transition-colors bg-indigo-500/10 hover:bg-indigo-500/20 px-2 py-1 rounded border border-indigo-500/20 cursor-pointer"
          ...
          <div className="flex-1 min-h-[480px] relative rounded-3xl border border-white/10 overflow-hidden flex flex-col group shadow-2xl transition-colors duration-300" 
            ...
            <div className="absolute bottom-4 right-6 pointer-events-none z-10">
              <div className="bg-[#0a0a0a]/80 backdrop-blur border border-white/10 rounded-lg py-1 px-2.5 pointer-events-auto">
                <code className="text-[10px] font-mono text-indigo-455">
                  {`<BGPattern size={${size}} />`}
                </code>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="border-t border-white/10 pt-3 mt-1 text-xs">
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <span className={`text-[10px] font-mono font-medium px-2 py-0.5 rounded \${showGlow ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'bg-zinc-805 text-zinc-400'}`}>
              ...
              <div className="border-t border-white/10 pt-3 mt-1 text-xs">
                ...
                <div className="flex items-center justify-between gap-4 mb-2">
                  <span className="text-[11px] text-slate-500 font-sans font-medium text-right">اعوجاج عدسی مقعر اطراف اشاره‌گر ماوس (با جابجایی تجمعی نقطه‌ها)</span>
                  <label className="relative inline-flex items-center cursor-pointer select-none">
                    <input 
                      type="checkbox" 
                      checked={interactive} 
            ...
            <div className="border-t border-white/10 pt-4 mt-2">
              ...
              <div className="bg-[#0f0f12] border border-white/5 rounded-2xl p-4 space-y-4">
                <p className="text-[11px] text-slate-400 font-sans leading-relaxed text-right animate-pulse" style={{ direction: 'rtl' }}>
                    ...
                    <button
                      ...
                      className={`text-[9px] py-1 border rounded-lg transition-all text-center leading-normal break-words \${
                        exportWidth === preset.w && exportHeight === preset.h
                          ...
                          : 'bg-white/5 border-white/5 hover:bg-white/10 text-slate-355'
                      }`}
                    >
                      {preset.label}
                    </button>
                ...
                <div className="flex items-center gap-3 pt-1">
                  <div className="flex bg-[#08080a] border border-white/10 rounded-xl p-0.5">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 text-[10px] font-mono font-bold rounded-lg transition-all \${
                        exportFormat === 'jpeg' ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-slate-400'
                      }`}
                    >
                      JPG
                    </button>
                ...
                {activeTab === 'preview' && (
                  ...
                  >
                    ...
                    <div className="space-y-2 text-xs text-zinc-400">
                      ...
                      <ul className="list-disc pl-4 space-y-1.5 text-[11px] leading-relaxed text-right" style={{ direction: 'rtl' }}>
                        <li>
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            className={`fixed bottom-6 right-6 z-50 flex items-center gap-2.5 px-5 py-3.5 rounded-2xl border shadow-2xl font-sans text-xs font-semibold backdrop-blur-md ${
              toastType === 'success'
